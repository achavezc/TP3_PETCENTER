...
function ObtenerSucursalesByLinea(codLinea) {
    var resultado = [];
    $.ajax({
        url: "/Maestros/ListarSucursalByLinea",
        type: "POST",
        ...
        success: function (data) {
            if (data.SucursalesList.length > 0) {
                resultado = data.SucursalesList;
            }
...
function HtmlCrearUrl(EventoNgClick, valor) {
    return HtmlCrearUrl_Aux(EventoNgClick, valor);
}
function HtmlCrearBoton_Aux(tipo, valor, EventoNgClick, atributosHtmlBoton) {
    ...
    if (tipo == "Estado") {
        salida = '<div style="width:100%;padding-left:0px"><a style="cursor:pointer"><img width="16" ' + atributosHtmlBoton + ' height="16" src="/images/' + imgEstado + '" /></a></div>';
    }
    else {
        salida = '<div style="width:100%;padding-left:10px"><a style="cursor:pointer;width:100%"><button title="' + strTooltip + '" ' + atributosHtmlBoton + ' ng-click="' + EventoNgClick + '" class="boton1Style botonpequenio"><img width="16" height="16" src="/images/' + strImg + '"></button></a></div>';
    }

    return salida;
}
function HtmlCrearUrl_Aux(EventoNgClick, valor) {


    salida = '<div style="width:100%"> <button style="width: 80%;" ng-click="' + EventoNgClick + '" >' + valor + ' </button></div>';
...
function JqGridCrearBtn(cellvalue, options, tipo, rowId, rowId2, IdSufijo, rowId3, rowObject, nameId1) {
    ...
    if (tipo === "Modificar") {
        salida = '<div style="width:100%"><a style="cursor:pointer;width:100%"><button title="Modificar" ng-click="api.ModificarGrid(\'' + IdSufijo + '\',\'' + rowId + '\',\'' + rowId2 + '\',\'' + rowId3 + '\',\'' + nameId1 + '\')" dataval="' + rowId + '" ' + dataval2 + dataval3 + ' class="boton1Style botonpequenio btnJqgridModificar' + _Idsufijo + '"><img width="16" height="16" src="/images/icoModify.png"/></button></a></div>';
        ...
        var miimage = '<img alt="Responsable" style="padding-right:0px;border-width: 0px;" class="imageButton" src="/images/buscar.png"/>';
        ...
        salida = '<div style="width:100%;padding-left:0px"><a style="cursor:pointer"><img width="16" height="16" src="/images/"' + imgEstado + ' alt="" dataval="' + rowId + '" ' + dataval2 + '/></a></div>';
    }
...
getPopupResponsive = function (m_settings) {
    ...
    $.ajax({
        ...
        success: function (datos) {
            ...
            objBox.modal(show = true, backdrop = false);
            objBox.draggable({
