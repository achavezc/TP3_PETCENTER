		...
		.controller('LayoutController',
			...
			function ($scope, $http, $routeParams, $timeout, Enum, ParseHtml, $rootScope, Helpers, ServiciosConector, $window, $location) {
			    ...
			    $timeout(function () {
			        ...
			        if ($scope.Autenticado == true) {
			            if (window.location.hash.length <= 3) {
			                ...
			                $location.url("sistema/bienvenido/")
			    ...
			    $rootScope.CargaInicialAplicacion = function () {
			        $.ajax({
			            url: "/ModuloCalculadorWebAgma/CargaInicialAplicacion",
			            type: "POST",
			            ...
			            success: function (data) {
			                ...
			                if (data != undefined) {
			                    if (data.Resultado.Estado === true) {
			                        ...
			                        $rootScope.DatosFormulario.PermisosBotones = data.PermisosBotones;

			                    }
			                    else {
			                        MiError(data.Resultado.Estado);
			                    }
			                }
			            }
			        });
